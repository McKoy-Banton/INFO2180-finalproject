window.addEventListener('load', ()=>{
    ...
    saveBtn.addEventListener('click', (e) => {
        ...
        if(fieldsOK)
        {
            ...
            fetch('addContactFunction.php', {
                method: 'POST',
                headers: {'Content-Type':'application/x-www-form-urlencoded'},
                body: `title=${title.value}&firstname=${firstName.value.trim()}&lastname=${lastName.value.trim()}&email=${email.value.trim()}&telephone=${number.value}&company=${company.value}&type=${type.value}&assign=${assigned.value}`
            })
            .then(response => {
            ...
            .then(data => {
                ...
                if(data === "Contact added successfully to the database."){
